...
typedef struct Node{
    int id;//id号
    char partition[1024];//分区
    int start;//起始地址
    int end; //结束地址
    int size;//大小
    char state[1024];//当前的状态
}Node;
...
void Initialize(){
    ...
    for(int i = 0; i < 1000; i++) {
        strcpy(node[i].partition, "void");
        strcpy(node[i].state, "空闲");
    }
...
void distribution(){
    ...
    if(temp1 >= processSize){
        ...
        cnt++;
        node[cnt].id = cnt;
        strcpy(node[cnt].partition, processName);
        if(cnt == 1){
            node[cnt].start = 0;
        }else{
        ...
        */
        if(ch == 'Y'){
            ...
            for(int i = 1; i <= cnt; i++) {
                if(strcmp(node[i].partition, "void") == 0){
...
*/
    ...
    for(int i = 1; i <= cnt; i++) {
        if(strcmp(processName, node[i].partition) == 0){
    ...
    if(flag == 1){
        ...
        */
        for(int i = 1; i < cnt; i++){
            ...
            if(strcmp(node[i].partition, "void") == 0 && strcmp(node[i+1].partition, "void") == 0){
                ...
                for( ; j <= cnt; j++){
                    node[j-1].id = j - 1;
                    strcpy(node[j-1].partition, node[j].partition);
                    node[j-1].start = node[j].start;
                    node[j-1].end = node[j].end;
                    node[j-1].size = node[j].size;
                    strcpy(node[j-1].state, node[j].state);
                }
...
void show(){
    ...
    if (cnt == 0) {
        ...
        if(remainTemp > 0){
            cnt++;
            node[cnt].id = cnt;
            strcpy(node[cnt].partition, "void");
            if(cnt == 1){
                node[cnt].start = 0;
            }else{
                node[cnt].start = node[cnt-1].start + node[cnt-1].size;//本次的开始地址等于上次的开始地址加其大小
